...
export default function PreService() {
  ...
  async function deleteRow(id) {
    ...
    try { await api.deletePreservice(id); toast.success('Deleted'); load(page) }
    catch (e) { toast.error(e.message) }
  }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      <div className="dash-header">
        <div className="dh-left">
          <div className="dh-icon" style={{ background:'#f0fdf4', color:'#059669' }}>
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
          </div>
          <div>
        ...
        <div className="table-card">
          ...
          <div className="tbl-wrap">
            <table>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={9} style={{ textAlign:'center', padding:40 }}><div className="spinner dark" style={{ margin:'0 auto' }} /></td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={9}><div className="tbl-empty"><div className="tbl-empty-icon">ðŸ“š</div><div className="tbl-empty-title">No records found</div><div className="tbl-empty-sub">Import a CSV or add records manually</div></div></td></tr>
                ) : rows.map((r, i) => (
                  <tr key={r.id}>
      ...
      {showAdd && (
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget&&setShowAdd(false)}>
          <div className="modal-box">
            <div className="modal-header"><div className="modal-title">Add Pre-Service Record</div><button className="modal-close" onClick={()=>setShowAdd(false)}>âœ•</button></div>
