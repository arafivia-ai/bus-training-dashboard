...
function Tag({ v }) {
  ...
  const cls = (l.includes('pass')||l==='graduated')?'tag-green':l.includes('fail')?'tag-red':l.includes('progress')?'tag-amber':'tag-gray'
    ...
    <div className="space-y-5">
      ...
      {tab==='Overview' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KPICard label="Scenario Pass"     value={`${analytics?.scenarioPassRate||0}%`}    icon={BookOpen} color="amber"  sub="Scenario test"/>
              ...
              {passRates.map(({name,value}) => (
                <div key={name} className="space-y-1.5">
                  ...
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${value>=70?'bg-emerald-500':value>=50?'bg-amber-500':'bg-red-500'}`} style={{width:`${value}%`}}/>
                  </div>
                </div>
              ))}
            </div>
      ...
      {tab==='Analysis' && (
        <div className="space-y-4">
          ...
          <BarChartBox title="Pass Rates by Stage" sub="Percentage per stage"     data={passRates}  height={260} color="#059669"/>
        </div>
      )}
      ...
      {tab==='Records' && (
        <div className="card">
          ...
          <DataTable columns={columns} rows={filtered} loading={loading} page={page} total={total} limit={LIMIT} onPage={p=>loadRecords(p)} emptyIcon="📚" emptyText="No pre-service records found"/>
        </div>
      )}
    </div>
  )
