...
import { Filter, Upload, Plus, Search, Bus, X } from 'lucide-react'
...
export default function InService() {
  ...
  async function saveRecord() {
    ...
    try { await api.addInservice(form); toast.success('Record added'); setShowAdd(false); setForm({}); load(1) }
    catch(e) { toast.error(e.message) }
  }
  ...
  const filtered = search ? rows.filter(r=>(r.driver_name||'').toLowerCase().includes(search.toLowerCase())||(r.staff_id||'').includes(search)) : rows
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      <div className="dash-header">
        <div className="dh-left">
          <div className="dh-icon" style={{background:'#eff6ff',color:'#1d4ed8'}}><Bus size={20}/></div>
          <div><div className="dh-title">In-Service Training</div><div className="dh-sub">{total.toLocaleString()} total records</div></div>
        </div>
                ...
                {loading ? (
                  ...
                  <tr><td colSpan={10}>
                    <div className="tbl-empty">
                      <Bus size={36} style={{margin:'0 auto 12px',display:'block',color:'#cbd5e1'}}/>
                      <div className="tbl-empty-title">No records found</div>
                      <div className="tbl-empty-sub">Import a CSV or add records manually</div>
                    </div>
                  </td></tr>
                ) : filtered.map((r,i)=>(
                  <tr key={r.id}>
      ...
      {showAdd&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}>
          <div className="modal-box">
            <div className="modal-header"><div className="modal-title">Add Training Record</div><button className="modal-close" onClick={()=>setShowAdd(false)}><X size={18}/></button></div>
