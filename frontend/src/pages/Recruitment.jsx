...
export default function Recruitment() {
  ...
  const notShortlist = analytics?.byStatus?.['Not Shortlisted'] || 0
      ...
      {tab === 'Overview' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KPICard label="Not Shortlisted"    value={notShortlist}          icon={UserCheck} color="red"    sub="Not progressed"/>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <DonutChartBox title="Pipeline Status" sub="Candidates by current status" data={statusData} height={280}/>
      ...
      {tab === 'Records' && (
        <div className="card">
          ...
          <DataTable columns={columns} rows={filtered} loading={loading} page={page}
            total={total} limit={LIMIT} onPage={p => loadRecords(p)} emptyIcon="👥" emptyText="No candidates found"/>
        </div>
      )}
    </div>
  )
