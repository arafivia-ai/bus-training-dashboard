...
export default function Login() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      toast.success('Welcome, ' + (res.user?.username || username))
      navigate('/')
    } catch (err) {
      setFails(f => f+1)
    ...
    <div style={{display:'flex',height:'100vh',background:'#0f2044'}}>
      ...
      <div style={{width:460,display:'flex',alignItems:'center',justifyContent:'center',padding:40,background:'#fff'}}>
        <div style={{width:'100%',maxWidth:360}}>
          ...
          {error&&<div style={{background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:6,padding:'10px 14px',fontSize:12.5,color:'#dc2626',marginBottom:16}}>{error}</div>}
          ...
          <div style={{marginTop:18,fontSize:11.5,color:'#94a3b8',display:'flex',alignItems:'center',gap:6}}>
            <Lock size={12}/> Accounts lock after 5 failed attempts
          </div>
        </div>
      </div>
    </div>
