...
export default function ChartBox({ type, labels, data, title, sub, height=240, horiz=false }) {
  const ref = useRef()
  const inst = useRef()
  ...
  return (
    <div className="chart-card">
      <div className="cc-header">
        <div><div className="cc-title">{title}</div>{sub&&<div className="cc-sub">{sub}</div>}</div>
      </div>
